...
const campaignEntryList = (entryList) => {
    return entryList.reduce((acc, appName) => {
        acc[`${appName}`] = resolveApp(`./src/${appName}/index.js`);
        return acc;
    }, {});
};

// 自动扫描指定目录获取所有入口
function getEntries(directories = ['demos', 'main']) {
    const entries = {}
...
// return configFactory('production', item, Object.keys(item)[0]);
// })

console.log('getEntries =>', getEntries());
const config = configFactory('production', getEntries)
